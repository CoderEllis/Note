...
class PhotoBrowserController: UIViewController {
    ...
    var picURLs : [URL]
    //保存传送过来View属性用来计算frame
    var saveComingView : UICollectionView
    
    var stringUrls : [String]? {
        didSet {
            guard let url = stringUrls else {
                return
            }
            for i in 0..<url.count { //0-9  0...10 { // 0-10
    ...
    init(_ collectionView: UICollectionView, indexPath: IndexPath, picURLs: [URL], photoBrowserAnimator: PhotoBrowserAnimator, back: @escaping bgTransitionMask) {
        self.originFrameCallback = back
        self.indexPath = indexPath
        self.picURLs = picURLs
        self.saveComingView = collectionView
        super.init(nibName: nil, bundle: nil)
        
        transitioningDelegate = photoBrowserAnimator
        //设置modal样式 跳转方式
        ...
        if error != nil {
            ...
            let saveImageAction = UIAlertAction(title: "前往设置", style: .default) { (_) in
                ...
                if #available(iOS 10.0, *) {
                    ...
                    let success = UIApplication.shared.openURL(settingUrl!)
                    print("Open \(success)")
                }
...
//这种固定获取 index 有限  动态的话还是谁代理谁计算吧
extension PhotoBrowserController :AnimatorPresentedDelegate { 
    func startRect(_ indexPath: IndexPath) -> CGRect {
        let cell = saveComingView.cellForItem(at: indexPath)!
        // 2.获取cell的frame
        let startFrame = saveComingView.convert(cell.frame, to: UIApplication.shared.keyWindow)
